...
class Chapter9RegistrationTests(TestCase):
    ...
    def test_registration_get_response(self):
        ...
        self.assertTrue('<p><label for="id_password">Password:</label><input type="password" name="password" required id="id_password"></p>' in content, f"{FAILURE_HEADER} [[[ THIS TEST IS WRONG AND SHOULD PASS ]]] Checking a random form field in register.html (password), the markup didn't match what we expected. Is your password form field configured correctly?{FAILURE_FOOTER}")
